...
- 新たなサブクラスを定義するたび別の継承ツリーにもサブクラスを定義しなければならない状況
- 「変更の偏り」の特殊ケース
- 一方の継承ツリーのインスタンスが、もう一方の継承ツリーのインスタンスを参照するようにする
- 参照している側では「メソッドの移動」「フィールドの移動」を行い、継承構造を廃止していく

### 怠け者クラス

- 十分な仕事をしていないクラスは排除するべき
- サブクラスが十分な働きをしていない場合
  - 「階層の平坦化」
- ほとんど役に立たないヘルパークラス
  - 「クラスのインライン化」
...
- 現在は必要としていない凝った仕掛けや特殊な状況を考えている場合
- 無用の長物なら削除したほうがまし
- 大した働きをしない抽象クラスは「階層の平坦化」
- 意味のない委譲は「クラスのインライン化」
- 使われないパラメータを持つメソッドについては「パラメータの削除」
- わかりにくい抽象的な名前のメソッドは「メソッド名の変更」
- テストケースでしか使われていないメソッドは疑う

### 一時的属性

- インスタンス変数の値が特定の状況でしか設定されないオブジェクト
- 「クラスの抽出」を使って、変数の居場所を作る
- 「ヌルオブジェクトの導入」を使って変数の値が代入されないときに代わりに入れる

### メッセージの連鎖

- メソッドの連鎖が起きているような場合
  - getXXXのようなメソッド呼び出しが長い行となって連なっている
  - 一時変数が非常に沢山定義されている
- 「委譲の隠蔽」を行う
- 仲介人になってしまいがち
- 「メソッドの抽出」と「メソッドの移動」で連鎖の端の方へ移動

### 仲介人

- カプセル化が権限の委譲をもたらす
  - 上司にスケジュールを聞くと手帳を確認する
  - 聞く先は携帯でも秘書でもいい
- 委譲が過剰になると仲介人になってしまう
- 「仲介人の除去」を適用して直接処理させる
- メソッドがわずかなら「メソッドのインライン化」を適用
- なにか処理が加わっているなら「継承による委譲の置き換え」

### 不適切な関係

- クラスの仲が良すぎて、プライベートな部分を探り出すのに苦労する
- 遠くに離してあげる
  - 「メソッドの移動」「フィールドの移動」
  - 「双方向関連の単方向への変更」
- 共通部分がある
...
- 処理は同じでシグニチャのみが異なるメソッド
  - 「メソッド名の変更」
  - 「メソッドの移動」
- 重複したコードが発生するなら「スーパークラスの抽出」

### 未熟なクラスライブラリ

- クラスライブラリも全能ではない
- 追加したいメソッドがわずかなら「外部メソッドの導入」
- 追加される振る舞いが多いなら「局所的拡張の導入」
...
- できるだけ早く「フィールドのカプセル化」
- 必要に応じ「コレクションのカプセル化」
- 変更されては困る属性は「setメソッドの削除」
- getやsetの使用箇所を見て、振る舞いをデータクラスに移動する
  - 「メソッドの移動」
  - 「メソッドの抽出」
  - getやsetに対して「メソッドの隠蔽」
...
- 親の属性と操作をサブクラスが一部しか使用していない
- 継承階層が間違っている
  - 「メソッドの引き下げ」
  - 「フィールドの引き下げ」
  - 常に対処しなくてもよい
    - 振る舞いの再利用だけの継承でうまくいくことも多い
- 振る舞いは継承するがインターフェースは必要でない
  - 「委譲による継承の置き換え」

### コメント

- コメントが消臭剤として使われることがある
  - 不吉な臭いの予兆
- 処理を説明するのにコメントが必要
  - 「メソッドの抽出」
  - 「メソッド名の変更」
...
- 良いテストを書くとプログラミングが加速する
- テストの構築だけではなく、それを頻繁に実行することが絶大な力を発揮する
- リファクタリングにはテストが必須
...
- 単体テストはプログラマの生産性を向上するため
- 機能テストはソフトウェア全体の動作を保証するため
  - 顧客に対する品質保証
  - 可能な限りブラックボックス
- バグレポートを受け取ったらまずバグを明らかにする単体テストを書く
- リファクタリングには単体テストが有効
...
- テストはリスク主導であるべき
- たくさん書けばいいというものではない
- 大事なのは一番怪しいと思う部分をテストすること
- 失敗の恐れのある境界条件を集中的にテストする
- 予想されるエラー（例外）もテストする
- 選択肢が互いに十分独立であれば、各組み合わせをテストしない
- 何年もかけてすべてのバグを補足するより、適切な時間で大部分のバグを補足する
